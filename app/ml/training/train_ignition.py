...
def group_split(X, y, fire_ids, val_frac=0.2, seed=0):
    ...
    rng.shuffle(fires)
    n_val = max(1, int(len(fires)*val_frac))    # max(1,...) ensures at least 1 fire event placed in validation set
    val_fires = set(fires[:n_val].tolist())
    val_mask = np.isin(fire_ids, list(val_fires))   # Checks fire_ids array, creates bool mask to check if each row's fire ID belongs to validation set
    return (X[~val_mask], y[~val_mask], X[val_mask], y[val_mask], sorted(val_fires))    # '~' bitwise NOT, flips bools to pull training data (X[~val_mask], y[~val_mask]). Pulls out validation data (X[val_mask], y[val_mask])
...
def train(X_train, y_train, X_va, y_va, device: str, seed: int=0) -> xgb.Booster:
    ...
    neg = len(y_train) - pos
    spw = neg/pos   # spw = scale_pos_weight. Calculate ratio of negative examples to positive examples. spw val fed into xgboost config
    ...
    return xgb.train(params, dtrain, num_boost_round=600, evals=[(dtrain, "train"), (dval, "val")], early_stopping_rounds=40, verbose_eval=50,)

def main() -> None:
    # CLI
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--data", default="synthetic", help="'synthetic' or path to aligned .npz (X, y, fire_ids)")
    ap.add_argument("--device", default="cpu", help="cpu | cuda | cuda:0 (per machine: each host has 1 GPU)")
    ap.add_argument("--val-frac", type=float, default=0.2)
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--promote", action="store_true", help="Point LATEST at this run after publishing")
    args = ap.parse_args()
    
    # Execution pipeline
    X, y, fire_ids = load_dataset(args.data)
    X_train, y_train, X_va, y_va, val_fires = group_split(X, y, fire_ids, args.val_frac, args.seed)
    ...
    print(f"Published: ignition/{version}" + (" (promoted to LATEST)" if args.promote else " (not promoted. Use artifact_store.promote_version to ship it)"))

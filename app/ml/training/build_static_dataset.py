...
from app.ml.features.weather_grid_loader import SA_LAT_MAX, SA_LAT_MIN, SA_LON_MAX, SA_LON_MIN, build_sa_grid_shape
...
def build_static_tensor(cfg: StaticDatasetConfig = StaticDatasetConfig()) -> Path:
    ...
    static_tensor = np.stack([
        ...
        np.sin(aspect_rad).astype(np.float32), np.cos(aspect_rad).astype(np.float32),
    ])
    out_path.parent.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(out_path, static_tensor=static_tensor)
    print(f"Saved {out_path}: static_tensor {static_tensor.shape} (channels: {STATIC_CHANNELS})")
    return out_path
...
def main() -> None:
    ...
    ap.add_argument("--resolution-deg", type=float, default=StaticDatasetConfig.resolution_deg)
    args = ap.parse_args()
    cfg = StaticDatasetConfig(dem_path=args.dem_path, out_path=args.out_path, resolution_deg=args.resolution_deg)

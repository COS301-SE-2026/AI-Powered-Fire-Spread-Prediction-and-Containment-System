...
def make_static(rng, H, W) -> dict[str, np.ndarray]:
    ...
    def smooth(field, passes=8):
        for i in range(passes):
            field = (field + shift(field, 0, 1, 0) + shift(field, 0, -1, 0) + shift(field, 1, 0, 0) + shift(field, -1, 0, 0)) / 5.0
        return field
...
def make_weather(rng, H, W) -> dict[str, np.ndarray]:
    ...
    return{
        ...
        "rel_humidity": np.clip(rng.normal(35, 12) + rng.normal(0, 3, (H, W)), 5, 95).astype(np.float32),
        "temperature": np.clip(rng.normal(28, 6) + rng.normal(0, 1.5, (H, W)), 5, 48).astype(np.float32),
    }
...
def generate_synthetic_dataset(cfg: SynthConfig = SynthConfig()) -> tuple[np.ndarray, np.ndarray, np.ndarray]:

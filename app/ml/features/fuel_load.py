...
def worldcover_s3_urls_for_bbox(
    min_lon: float, min_lat: float, max_lon: float, max_lat: float, year: int=2021, 
) -> list[str]:
    ...
    if year not in WORLDCOVER_VERSIONS:
        raise ValueError(f"No known WorldCover version for year={year}"
...
def _resample_to_shape(arr: np.ndarray, transform, crs, out_shape: tuple[int, int], resampling: Resampling) -> np.ndarray:
...
def _read_worldcover_window(
    ...
    urls = worldcover_s3_urls_for_bbox(min_lon, min_lat, max_lon, max_lat, year=worldcover_year)
...
def process_sentinal2_and_worldcover(
    ...
    with rasterio.Env(**S3_READ_ENV):
        ...
        def read_band(path: str) -> np.ndarray:
            return _read_window_single(
                path, min_lon, min_lat, max_lon, max_lat, (H, W), Resampling.bilinear,
            )
        ...
        if scl_path is not None:
            scl = _read_window_single(
                scl_path, min_lon, min_lat, max_lon, max_lat, (H, W), Resampling.nearest,
            )

...
test.describe('E2E test for offline map cache and synchronisation after returning online', () => {
    ...
    test('Live incidents should be loaded and populate the IndexedDB cache when online', async ({ page }) => {
        await page.goto('/');
    ...
    test('Display offline indicator when network drops and render cached fire records', async ({
        ...
        await expect(offlineBar).toContainText('Offline! You are viewing outdated incidents and predictions');
    });

  ...
  return (
    <div className="min-h-screen bg-[#080B12] flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#12151E] border border-white/10 rounded-xl p-6">
        <h2 className="text-2xl font-bold text-[#EDEAE5] text-center mb-6">Welcome back</h2>
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          {error && <div className="bg-red-500/10 border border-red-500/50 text-red-400 text-sm p-2 rounded">{error}</div>}
          ...
          <Link href="/register" className="block w-full py-2 text-center border border-white/20 rounded-md hover:bg-white/5 transition">
            Register
          </Link>
        ...
        <div className="text-center mt-4 text-sm text-white/40">
          ...
          <Link href="/cant-login" className="hover:text-red-400"> Can't log in?</Link>
        </div>
      </div>
    </div>
  );

...
export default function ReportDetailsForm({ location = "", onSubmit, onLocationSearch }: Props) {
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        if (!validLocationRef.current || !editableLocation.trim() || editableLocation === LOCATION_PLACEHOLDER) {
    ...
    return (
        <form onSubmit={handleSubmit} className="w-full flex flex-col font-body">
            ...
            <div className="flex flex-col gap-5">
                ...
                <div className="mt-1">
                    <button type="submit" className="btn w-full h-11 bg-ignite border-none text-white font-display font-bold tracking-widest uppercase text-lg">Submit Fire Report</button>
                </div>
            </div>
        </form>
    );

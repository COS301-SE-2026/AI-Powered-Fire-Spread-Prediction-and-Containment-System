...
interface MyFireReportsTableProps {
...
export function MyFireReportsTable({ reports, filter }: MyFireReportsTableProps) {
  ...
  const basePath = ROLE_PATHS[user?.role ?? 'user'] ?? 'users';

  return (
    <div className="w-full overflow-x-auto overflow-y-auto max-h-[400px] rounded-2xl border border-carbon-stroke">
            ...
            filtered.map((report) => (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <button
                    type="button"
                    onClick={() => router.push(`/${basePath}/${report.reference_number}`)}

...
export function MapPanel({children, colSpan=8, height='lg'}: MapPanelProps){
    ...
    return(
        <div className={`${colSpanClass} flex flex-col gap-6`}>
            <div className={`relative rounded-2xl overflow-hidden border border-surface-card ${heightClass} w-full shadow-md`}>
                {children}
            </div>
        </div>
    );

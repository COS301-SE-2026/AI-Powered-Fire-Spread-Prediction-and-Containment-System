...
def test_return_report(client, mock_db, mock_report):
    mock_db.query.return_value.outerjoin.return_value.all.return_value = [ 
        (mock_report, valid_payload["lat"], valid_payload["lng"]) 
        ]
    response = client.get("/api/guests/reported-fires")
...
def test_multiple_returns(client, mock_db, mock_report):
    ...
    mock_report_2.status = ReportStatus.received
    mock_report_2.status_index = ReportStatus.received.value
    mock_report_2.submitted_at = datetime(2025, 1, 2, 12, 0, 0)
    mock_report_2.verification_notes = None
    mock_report_2.priority = "normal"
    mock_db.query.return_value.outerjoin.return_value.all.return_value = [ 
        (mock_report, valid_payload["lat"], valid_payload["lng"]), 
        (mock_report_2, -33.9249, 18.4241) 
        ]
    response = client.get("/api/guests/reported-fires")
    assert response.status_code == 200
...
def test_create_report(client, mock_db, mock_report):
    ...
    with patch("app.backend.src.services.users.fire_report.FireReports") as MockModel:

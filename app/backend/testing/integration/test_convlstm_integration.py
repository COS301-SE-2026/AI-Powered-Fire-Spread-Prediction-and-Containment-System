...
def test_convlstm_training_pipeline_integration(integration_env, monkeypatch):
    ...
    assert trainer.best_val_loss < float("inf"), "Validation loss should have decreased and updated"

    artifacts_path = Path(integration_env["artifacts_dir"])
    assert (artifacts_path / "model.pt").exists(), "Model weights were not saved to disk"
    assert (artifacts_path / "metadata.json").exists(), "Metadata JSON was not saved to disk"

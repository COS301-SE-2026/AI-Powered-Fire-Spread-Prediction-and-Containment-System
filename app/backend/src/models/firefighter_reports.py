...
class Firefigter_FireReports(Base):
    ...
    status = Column(Enum(FirefighterReportStatus), default=FirefighterReportStatus.received, nullable=False)
    status_index = Column(Integer, default=0, nullable=False)
    reported = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))

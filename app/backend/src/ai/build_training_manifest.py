...
def dem_vsis3_path(
    ...
    if (
        ...
        or max_lat > tile_lat + 0.95
        or min_lon < tile_lon + 0.05
        or max_lon > tile_lon + 0.95
    ):
        print(
            f"Warning bbox ({min_lon:.3f}, {min_lat:.3f})-({max_lon:.3f},{max_lat:.3f})"
            f"is close to the DEM tile boundary"
        )

    return path


def fetch_sent2_fire(
    fire_id: int,
    min_lon: float,
    min_lat: float,
    max_lon: float,
    ...
    if not items:
        print(
            f"skip fire id: {fire_id}: no sent 2 scene under the max cloud cover of {max_cloud_cover}%"
        )
        return None

    item = items[0]
    print(
        f"Fire ID: {fire_id} | using scene {item.id} with cloud cover {item.properties.get('eo:cloud_cover'):.1f}%"
    )
    ...
    for asset_key, suffix in [("red", "b04"), ("nir", "b08"), ("swir16", "b11")]:
        ...
        with httpx.stream(
            "GET", item.assets[asset_key].href, follow_redirects=True, timeout=60.0
        ) as r:
            r.raise_for_status()
...
def build_manifest(
    ...
    with open(safe_out_path, "w", newline="") as fire:
        ...
        for event in selected:
            ...
            sentinel_paths = fetch_sent2_fire(
                event.fire_id,
                event.min_lon,
                event.min_lat,
                event.max_lon,
            ...
            dem_path = dem_vsis3_path(
                event.min_lon, event.min_lat, event.max_lon, event.max_lat
            )

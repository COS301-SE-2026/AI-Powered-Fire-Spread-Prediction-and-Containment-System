...
class IgnitionScorer:   # The only interface backend/simulation loop needs
    ...
    def load(cls, version: str = "LATEST") -> "IgnitionScorer":
        ...
        if metadata.get("features") != FEATURES or metadata.get("schema_version") != SCHEMA_VERSION:
            raise RuntimeError(
    ...
    def score_grid(self, weather: dict[str, np.ndarray], static: dict[str, np.ndarray], burn_state: np.ndarray) -> np.ndarray:

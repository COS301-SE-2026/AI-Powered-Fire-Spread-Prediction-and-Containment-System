...
def publish(model_family: str, model_path: str | Path, metadata: dict, promote: bool = False) -> str:
    ...
    shutil.copy2(model_path, vdir / "model.json")   # Copies newly trained model file from temporary training location to new permanent version folder called model.json
...
def promote_version(model_family: str, version: str) -> None:   # Actually "promptes" a specific model version to be the active/LATEST version
...
def list_versions(model_family: str) -> list[str]:
    ...
    if not family_dir.is_dir():
        return []
    return sorted(d.name for d in family_dir.iterdir() if d.is_dir() and d.name.startswith("v"))
...
class HistoricalWeatherProvider:
    ...
    def fetch(self, min_lon: float, min_lat: float, max_lon: float, max_lat: float,
              timestamp, target_shape: tuple[int, int]) -> dict[str, np.ndarray]:
        raise NotImplementedError(
            "Connect to hitorical weather data"
        )
...
class OpenMeteoWeatherProvider(HistoricalWeatherProvider):
    ...
    def prepare_for_fire(self, event: "FireEvent", target_shape: tuple[int, int]) -> None:
        from app.datasets.scripts.fetch_historical_weather import fetch_historical_weather
 
        center_lat = (event.min_lat + event.max_lat) / 2.0
        center_lon = (event.min_lon + event.max_lon) / 2.0
        start = (pd.Timestamp(event.ticks[0]) - pd.Timedelta(days=self.date_buffer_days)).strftime("%Y-%m-%d")
        end = (pd.Timestamp(event.ticks[-1]) + pd.Timedelta(days=self.date_buffer_days)).strftime("%Y-%m-%d")
    ...
    def fetch(self, min_lon, min_lat, max_lon, max_lat, timestamp, target_shape) -> dict[str, np.ndarray]:
        from app.datasets.scripts.fetch_historical_weather import get_weather_at_timestamp
 
        if self._current_fire_id is None or self._current_fire_id not in self._df_by_fire:
            raise RuntimeError(
...
def load_static_grids_for_fire(manifest_row: dict, min_lon, min_lat, max_lon, max_lat, target_shape) -> dict[str, np.ndarray]:
    ...
    return {
        ...
        "slope": terrain["slope"],
        "aspect_sin": terrain["aspect_sin"] if "aspect_sin" in terrain else np.sin(np.radians(terrain["aspect"])),
        "aspect_cos": terrain["aspect_cos"] if "aspect_cos" in terrain else np.cos(np.radians(terrain["aspect"])),
...
def load_detections(csv_path: str, lat_col: str, lon_col: str, date_col: str, time_col: Optional[str]) -> pd.DataFrame:
    ...
    if time_col and time_col in df.columns:
        ...
        ts = pd.to_datetime(df[date_col]) + pd.to_timedelta(
...
def main() -> None:
    ...
    ap.add_argument("--csv", required=True, help="raw historical fire detections, e.g. fire_nrt_J2V-C2_77.csv")
    ...
    detections = load_detections(args.csv, args.lat_col, args.lon_col, args.date_col, args.time_col)
    print(f"Loaded {len(detections):,} raw detections from {args.csv}")
 
    fire_ids = cluster_fire_events(detections, max_gap_km=args.max_gap_km, max_gap_days=args.max_gap_days)
    n_fires = len(np.unique(fire_ids))
    print(f"Clustered into {n_fires} distinct fire events "

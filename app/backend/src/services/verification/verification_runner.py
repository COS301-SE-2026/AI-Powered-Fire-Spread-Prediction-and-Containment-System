...
from auto_verification import auto_verify_report, AUTO_REJECT, AUTO_VERIFY, MANUAL_REVIEW
...
def run_verification(report_id: str) -> None:
    ...
    try:
        ...
        db.commit()
        logger.info("Report %s auto-verification decision: %s (%s)", report_id, decision, reason)
    except Exception:
        logger.info("run_verification failed for report %s", report_id)
        db.rollback()
    finally:
        db.close()

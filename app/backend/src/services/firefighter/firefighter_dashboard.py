...
def get_nearby_fires(db: Session, lat: float, lng: float, radius_km: float = 20):
    ...
    request=db.query(FireReports).filter(ST_DWithin(geom_geog, point_geog, radius_km * 1000))\
        .add_columns(ST_Distance(FireReports.location_geom, point_geog))\
            .order_by(ST_Distance(geom_geog, point_geog))\
                .all() # * 1000 because ST_DWithin uses meters not km
...
def calculate_fire_danger(temp: float, humidity: float, wind: float):
    # need to find a calculation to determine fire risk will happen
    # when model for AI is more researched will use
    # XGboost for now acording to meetings
    return "high"

def get_current_environment_vars(lat:float, lng: float):
    # pings the open-meteo api every time this func
    # is called will look at making it real-time later in project
    ...
    return {
        ...
        "fire_danger": calculate_fire_danger(data["apparent_temperature"],
